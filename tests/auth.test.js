...
const testIp = () => `10.10.${Math.floor(Math.random() * 200) + 1}.${Math.floor(Math.random() * 200) + 1}`;
const post = (path) => request(BASE_URL).post(path).set('X-Forwarded-For', testIp());
const get = (path) => request(BASE_URL).get(path).set('X-Forwarded-For', testIp());
